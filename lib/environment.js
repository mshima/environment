...
const {createYoResolveTransform, createCollisionCheckerTransform, createYoRcTransform} = require('./util/transform');
...
class Environment extends EventEmitter {
  ...
  static createEnv(args, options, adapter) {
    return new Environment(args, options, adapter);
  }
   ...
   */
  constructor(args, options, adapter) {
    super();
    ...
    this.sharedOptions.forwardErrorToEnvironment = true;
  ...
  instantiate(Generator, options) {
    ...
    const options_ = options.options || this.options;
  ...
  async run(args, options, done) {
    ...
    if (!name) {
      throw Promise.reject(
        new Error('Must provide at least one argument, the generator namespace to invoke.')
      );
    }
  ...
  runGenerator(generator) {
    const promise = new Promise((resolve, reject) => {
      if (this.conflicter === undefined) {
        this.conflicter = new Conflicter(this.adapter, generator.options.force, {
          bail: this.options.bail || generator.options.bail,
          ignoreWhitespace: this.options.whitespace || generator.options.whitespace,
          skipRegenerate: this.options.skipRegenerate || generator.options.skipRegenerate,
          dryRun: this.options.dryRun || generator.options.dryRun
        });

        this.queueConflicter();

        // Register .yo-resolve file
        const yoResolve = this.options.yoResolve || generator.options.yoResolve;
        if (!this.options.skipYoResolve && !generator.options.skipYoResolve) {
          this.registerTransformStream(createYoResolveTransform(this.conflicter, yoResolve));
        }
      }
      ...
      if (generator.queueTasks) {
        /*
         * Yeoman-generator >= 5, just queue the tasks.
         * It should emit errors to environment directly.
         */
        generator.queueTasks();
      } else {
        /*
         * Root generator always forwarded to reject.
         * Listening to generator's error events was removed for generator v5.
         * We can't guarantee that a composed generator is been listened.
         */
   ...
   * @return {Promise}
   */
  commitSharedFs(stream) {
    return new Promise((resolve, reject) => {
      const transformStreams = [
        ...(this._rootGenerator && this._rootGenerator._transformStreams),
        ...this._transformStreams,
        createYoRcTransform(),
        createYoResolveTransform(this.conflicter),
        createCollisionCheckerTransform(this.conflicter)
      ];
  ...
  registerTransformStream(transformStream) {
    this._transformStreams.push(transformStream);
  }

  /**
   * Queue environment's basic tasks.
   */
  queueConflicter() {
    const writeFiles = () => {
      this.runLoop.add('conflicts', (done, stop) =>
        this.commitSharedFs().then(() => {
          this.sharedFs.once('change', writeFiles);
          done();
        }
        , stop)
      , {
